...
而 μ 和 σ 是整行一起算出來的 —— 所以你動任何一格，整行的輸出都會跟著變。
反向就必須把這個牽連還回去。`;
    breakAfter();
...
第三項 xn ⊙ E[d ⊙ xn] 是**扣掉與自己方向相關的部分** —— 因為前向除過 σ。`;
    ...
    breakAfter();
    commentary(wt)`
後兩項合起來的效果很值得記：**Layer Norm 會把梯度裡「整行一起變大」和
「整行一起放大」的成分扣掉**。

換句話說，它不准反向傳播去調整整行的平均值和尺度 —— 因為那兩件事在前向就被歸一化吃掉了，
調了也沒用。梯度只剩下真正會改變「行內相對關係」的那部分。

這也是為什麼 Layer Norm 讓訓練穩定：它同時在前向和反向都把尺度問題擋掉了。

還有一點跟 MLP 對照著看：這裡和 attention 一樣，**整行是綁在一起的** ——
