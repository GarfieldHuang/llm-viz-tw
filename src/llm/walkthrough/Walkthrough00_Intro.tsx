...
export function walkthroughIntro(args: IWalkthroughArgs) {
    ...
    let c0 = commentary(wt, null, 0)`歡迎來到GPT大語言模型的演練！在這裡，我們將探索僅為85,000個參數的 _nano-gpt_ 模型。

它的目標很簡單：獲取六個字母的序列： ${embed(ExampleInputOutput)}
並按字母順序對它們進行排序，即 "ABBBCC".`;
    ...
    let c5 = commentary(wt)`在3D檢視中，每個綠色單元格代表一個正在處理的數字，每個藍色單元格代表一個權重。${embed(GreenBlueCells)}
    序列中的每個數字首先被轉化為48個元素向量（根據本特定模型選擇的大小）。這就是所謂的_嵌入_(_embedding_)。`;
    breakAfter(c5);
    ...
    breakAfter();
    commentary(wt)`然後，嵌入(embedding)將穿過模型，經過一系列稱為轉換器(transformers)的層，最後到達底層。`;
    breakAfter();
