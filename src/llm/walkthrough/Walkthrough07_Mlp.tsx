...
export function walkthrough07_Mlp(args: IWalkthroughArgs) {
    ...
    commentary(wt)`
    在自注意力（self-attention）之後，變換器（transformer）模組的下半部分是 MLP（多層感知器）。這詞有點拗口，但在這裡它是一個有兩層的簡單神經網路。

    與自注意力（self-attention）一樣，在向量進入 MLP 之前，我們要進行${c_blockRef('層歸一化（layer normalization）', block.ln2.lnResid)}處理。

    在 MLP 中，我們將每個長度為 ${c_dimRef('C = 48', DimStyle.C)} 的列向量（獨立地）進行以下處理：

    1. 進行${c_blockRef('線性變換', block.mlpProjWeight)}並新增${c_blockRef('偏置', block.mlpProjBias)}，轉換為長度為 ${c_dimRef('4 * C', DimStyle.C4)} 的向量。

    2. 一個 GELU 啟用函式（元素級）

    3. 進行${c_blockRef('線性變換', block.mlpProjWeight)}並新增${c_blockRef('偏置', block.mlpProjBias)}，返回長度為${c_dimRef('C', DimStyle.C)}的向量
...
commentary(wt)`
我們首先進行帶偏置的矩陣-向量乘法運算，將向量擴展到長度 ${c_dimRef('4 * C', DimStyle.C4)}。（注意，這裡輸出矩陣是轉置的。這純粹是為了視覺化目的。）
`;
    breakAfter();
...
commentary(wt)`
接下來，我們對向量的每個元素應用 GELU 啟用函式。這是任何神經網路的關鍵部分，我們在模型中引入了一些非線性。所使用的特定函式 GELU，看起來很像 ReLU 函式（計算為 ${<code>max(0, x)</code>}），但它有一個平滑的曲線，而不是一個尖角。
...
commentary(wt)`
然後，我們透過另一個帶偏置的矩陣-向量乘法將向量投射回長度${c_dimRef('C', DimStyle.C)}。
`;
    breakAfter();
...
commentary(wt)`
與自注意力+投影部分類似，我們將MLP的結果元素級地新增到其輸入中。
`;
    breakAfter();
...
commentary(wt)`
至此，MLP 完成。現在我們有了變換器（transformer）模組的輸出，可以將其傳遞給下一個模組了。
`;
