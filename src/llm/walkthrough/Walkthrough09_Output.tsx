...
"logits"這個名字來源於 "log-odds"，即每個 token 的機率的對數。之所以使用 "對數"，
...
當我們對模型進行時間步進時，我們會使用上一列的機率來決定下一個要新增到序列中的 token。
例如，如果我們已經向模型提供了 6 個 token，我們就會使用第 6 列的輸出機率。

這一列的輸出是一系列機率，我們實際上必須從中選出一個作為下一個機率。我們的做法是 "從分佈中取樣"。
也就是說，我們按照機率加權隨機選擇一個 token。例如，機率為 0.9 的 token 將在 90% 的情況下被選中。

不過，這裡還有其他選擇，比如總是選擇機率最高的 token。

我們還可以使用溫度參數來控制分佈的 "平滑度"。溫度越高，分佈越均勻，溫度越低，分佈越集中在機率最高的 token 上。
