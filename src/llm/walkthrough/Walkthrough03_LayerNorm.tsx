...
export function walkthrough03_LayerNorm(args: IWalkthroughArgs) {
    ...
    上一節中的  ${c_blockRef('_輸入嵌入_', state.layout.residual0)} 矩陣是我們第一個變換器（Transformer）模組的輸入。

    變換器（Transformer）模組的第一步是對該矩陣進行層歸一化處理。這是對矩陣每列的值分別進行歸一化的操作。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
    歸一化是深度神經網路訓練中的一個重要步驟，它有助於提高模型在訓練過程中的穩定性。

    我們可以分別看每一列，所以現在先關注(右邊圖)第4列(${c_dimRef('t = 3', DimStyle.T)})。`;
    ...
    breakAfter();
    commentary(wt)`
    我們的目標是使該列的平均值等於 0，標準差等於 1。為此，我們要找出該列的這兩個量（${c_blockRef('平均值(\u03bc)', ln.lnAgg1)}和${c_blockRef('標準差(\u03c3)', ln.lnAgg2)}），然後減去平均值，再除以標準差。
    其目標是使該列的平均值等於 0，標準差等於 1。為此，我們找到該列的平均值（μ）和標準差（σ），然後減去平均值併除以標準差。`;
    ...
    breakAfter();
    commentary(wt)`
    這裡我們使用 E[x] 表示平均值，Var[x] 表示方差（長度為 ${c_dimRef('C', DimStyle.C)} 的列）。方差就是標準差的平方。ε項(ε = ${<>1&times;10<sup>-5</sup></>})是為了防止除以零。

    我們在聚合層中計算並儲存這些值，因為我們要將它們應用於列中的所有值。

    最後，在得到歸一化值後，我們將列中的每個元素乘以學習${c_blockRef('權重 (\u03b3)', ln.lnSigma)}，然後加上${c_blockRef('偏置 (β)', ln.lnMu)} ，最終得到${c_blockRef('歸一化值', ln.lnResid)}。`;
    ...
    breakAfter();
    commentary(wt)`
    我們在${c_blockRef('輸入嵌入矩陣(input embedding matrix)', layout.residual0)}的每一列上執行這一歸一化操作，得到的結果就是${c_blockRef('歸一化後的輸入嵌入（normalized input embedding）', ln.lnResid)}，並將其傳遞給自注意力（Self-Attention）層。
`;
